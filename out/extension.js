...
function getConfiguration() {
    ...
    return {
        ...
        paths: {
            services: config.get('paths.services', 'internal/application/services'),
            usecases: config.get('paths.usecases', 'internal/domain/usecases'),
            entities: config.get('paths.entities', 'internal/domain/entities'),
            dtos: config.get('paths.dtos', 'internal/domain/dto'),
            repositories: config.get('paths.repositories', 'internal/infra/databases/repositories'),
            repositoryContracts: config.get('paths.repositoryContracts', 'internal/application/contracts/repositories'),
            controllers: config.get('paths.controllers', 'internal/presentation/controllers'),
            serviceFactories: config.get('paths.serviceFactories', 'internal/main/factories/services'),
            repositoryFactories: config.get('paths.repositoryFactories', 'internal/main/factories/repositories'),
            controllerFactories: config.get('paths.controllerFactories', 'internal/main/factories/controllers')
        }
    };
}
...
async function detectGoModuleName(workspaceRoot) {
...
async function loadAndProcessTemplate(templatePath, variables, extensionPath) {
    try {
...
async function createService(name, workspaceRoot, extensionPath) {
    try {
...
async function createRepository(name, workspaceRoot, extensionPath) {
    try {
        const snakeName = toSnakeCase(name);
        const moduleName = await detectGoModuleName(workspaceRoot);
        const config = getConfiguration();
...
async function createEntity(name, workspaceRoot, extensionPath) {
    try {
        const snakeName = toSnakeCase(name);
        const moduleName = await detectGoModuleName(workspaceRoot);
        const config = getConfiguration();
...
async function createController(name, workspaceRoot, extensionPath) {
    try {
        const snakeName = toSnakeCase(name);
        const moduleName = await detectGoModuleName(workspaceRoot);
        const config = getConfiguration();
...
async function createDTO(name, workspaceRoot, extensionPath) {
    try {
        const snakeName = toSnakeCase(name);
        const moduleName = await detectGoModuleName(workspaceRoot);
        const config = getConfiguration();
